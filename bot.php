	...
	try{
		...
		if (in_array($cmd, $key) and $users[$chat_id]["adm"] == "true" || $confi['dono'] == $from_id){
			...
			if ($confibot['manutencao'] == "false" ){
				...
				bot("sendMessage",array("chat_id"=> $chat_id , "text" => "*Ops, Estou em Manutencao no momento ou eu bug no sevidor!*","reply_to_message_id"=> $message['message_id'],"parse_mode" => 'Markdown'));
			}
		}
